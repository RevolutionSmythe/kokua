...
BOOL LLFeatureManager::loadFeatureTables()
{
	...
	if (os_string.find("Microsoft Windows XP") == 0)
	{
		...
		http_filename = llformat(FEATURE_TABLE_VER_FILENAME, "_xp", ViewerInfo::versionNumbers4().c_str());
	}
	else
	{
		filename = llformat(FEATURE_TABLE_FILENAME, "");
		http_filename = llformat(FEATURE_TABLE_VER_FILENAME, "", ViewerInfo::versionNumbers4().c_str());
	}
#else
...
void LLFeatureManager::loadGPUClass()
{
	...
	// second table is downloaded with HTTP
	std::string http_filename = llformat(GPU_TABLE_VER_FILENAME, ViewerInfo::versionNumbers4().c_str());
	std::string http_path = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, http_filename);
...
void fetch_feature_table(std::string table)
{
	...
	if (os_string.find("Microsoft Windows XP") == 0)
	{
		filename = llformat(table.c_str(), "_xp", ViewerInfo::versionNumbers4().c_str());
	}
	else
	{
		filename = llformat(table.c_str(), "", ViewerInfo::versionNumbers4().c_str());
	}
#else
	const std::string filename   = llformat(table.c_str(), ViewerInfo::versionNumbers4().c_str());
#endif
