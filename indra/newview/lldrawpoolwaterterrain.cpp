...
BOOL water_deferred_render = FALSE;

BOOL LLDrawPoolWaterTerrain::sSkipScreenCopy = FALSE;
BOOL LLDrawPoolWaterTerrain::sNeedsReflectionUpdate = TRUE;
BOOL LLDrawPoolWaterTerrain::sNeedsDistortionUpdate = TRUE;
LLColor4 LLDrawPoolWaterTerrain::sWaterFogColor = LLColor4(0.2f, 0.5f, 0.5f, 0.f);
LLVector3 LLDrawPoolWaterTerrain::sLightDir;

LLDrawPoolWaterTerrain::LLDrawPoolWaterTerrain() :
	LLFacePool(POOL_WATERTERRAIN)
{
...
//static
void LLDrawPoolWaterTerrain::restoreGL()
{
	
}

LLDrawPool *LLDrawPoolWaterTerrain::instancePool()
{
...
void LLDrawPoolWaterTerrain::prerender()
{
...
S32 LLDrawPoolWaterTerrain::getNumPasses()
{
...
void LLDrawPoolWaterTerrain::beginPostDeferredPass(S32 pass)
{
	beginRenderPass(pass);
	water_deferred_render = TRUE;
}

void LLDrawPoolWaterTerrain::endPostDeferredPass(S32 pass)
{
	endRenderPass(pass);
	water_deferred_render = FALSE;
}

//===============================
//DEFERRED IMPLEMENTATION
//===============================
void LLDrawPoolWaterTerrain::renderDeferred(S32 pass)
{
...
void LLDrawPoolWaterTerrain::render(S32 pass)
{
...
void LLDrawPoolWaterTerrain::renderOpaqueLegacyWater()
{
	LLVOSky *voskyp = gSky.mVOSkyp;
...
void LLDrawPoolWaterTerrain::renderReflection(LLFace* face)
{
	LLVOSky *voskyp = gSky.mVOSkyp;
...
void LLDrawPoolWaterTerrain::shade()
{
	if (!water_deferred_render)
	{
	...
	swTime = (F32)LLFrameTimer::getElapsedSeconds()*0.5f;
	...
	shader->uniform1f(LLViewerShaderMgr::WATER_TIME, swTime);
...

	{
			...
			iter != mDrawFace.end(); iter++)
		{
			...
			else if (gGLManager.mHasDepthClamp || water_deferred_render)
			{
				face->renderIndexed();
			}
			else
...
LLViewerTexture *LLDrawPoolWaterTerrain::getDebugTexture()
{
	return LLViewerFetchedTexture::sSmokeImagep;
}

LLColor3 LLDrawPoolWaterTerrain::getDebugColor() const
{
